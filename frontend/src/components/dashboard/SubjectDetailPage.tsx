    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div>
        ...
        <div className="space-y-3">
          {subject.topics
            ?.filter((t: any) => t.depth === 0 || !t.parentId)
            .map((topic: any) => {
              const stats = topicBreakdown.find(
                (tb: any) => tb.topicId === topic.id,
              );
              return (
                  ...
                  {topic.children?.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-3">
                      {topic.children.map((child: any) => (
                        <span
                          key={child.id}
                          className="px-3 py-1.5 rounded-xl bg-zinc-50 dark:bg-surface-800 text-xs font-medium"
                        >
                          {child.name}
                          {child.author && (
