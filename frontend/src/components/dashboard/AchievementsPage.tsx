...
export function AchievementsPage() {
  const [data, setData] = useState<{ earned: any[]; locked: any[] } | null>(
    null,
  );
  ...
  return (
    <div className="space-y-8 animate-fade-in">
          ...
          <div className="flex items-center gap-6 mb-4">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-brand-500 to-navy-600 flex items-center justify-center">
              <span className="font-display font-black text-2xl text-white">
                {levelData.global.level}
              </span>
            </div>
            <div className="flex-1">
              <p className="font-display font-bold text-lg">
                Poziom globalny {levelData.global.level}
              </p>
              <p className="text-sm text-zinc-500">
                {levelData.global.totalXp} XP łącznie
              </p>
              <div className="mt-2 progress-bar">
                <div
                  className="progress-bar-fill"
                  style={{
                    width: `${Math.round(levelData.global.progress * 100)}%`,
                  }}
                />
              </div>
              <p className="text-xs text-zinc-400 mt-1">
                {levelData.global.next - levelData.global.totalXp} XP do poziomu{" "}
                {levelData.global.level + 1}
              </p>
            </div>
          </div>
            ...
            {data.earned.map((a: any) => (
              <div key={a.slug} className="achievement-card">
                ...
                <div>
                  ...
                  {a.xpReward > 0 && (
                    <span className="xp-badge mt-1">+{a.xpReward} XP</span>
                  )}
                </div>
              </div>
            ))}
      ...
      {data.locked.length > 0 && (
        <div>
          <h2 className="font-display font-semibold mb-3">
            Do zdobycia ({data.locked.length})
          </h2>
          <div className="grid sm:grid-cols-2 gap-3">
