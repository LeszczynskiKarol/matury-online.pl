...
export const dashboardRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/", { preHandler: [app.authenticate] }, async (req) => {
    ...
    const [
      ...
      // Last session date per subject
      app.prisma.studySession.groupBy({
        by: ["subjectId"],
        where: { userId, status: "COMPLETED" },
    ...
    // Build lookup: subjectId → lastSessionAt
    const lastSessionMap = new Map(
      lastSessionPerSubject.map((s) => [s.subjectId, s._max.completedAt]),
    );
    ...
    for (const a of answers30d) {
      ...
      if (!accuracyByDay[dateKey])
        accuracyByDay[dateKey] = { total: 0, avgScore: 0 };
    ...
    return {
      ...
      subjectProgress: subjectProgress.map((sp) => ({
        ...
        lastSessionAt: lastSessionMap.get(sp.subjectId) || null,
      })),
