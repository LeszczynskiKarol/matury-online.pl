...
export async function sendStreakReminders() {
  ...
  for (const user of users) {
    ...
    const alreadySent = await prisma.emailLog.findFirst({
      where: { userId: user.id, type: 'streak_reminder', sentAt: { gte: today } },
    });
    ...
    try {
      const res = await ses.send(new SendEmailCommand({
        ...
        Message: {
          Subject: { Data: `🔥 ${user.name || 'Hej'}, Twoja seria ${user.currentStreak} dni jest zagrożona!` },
          Body: {
            Html: {
              Data: `
<div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
  ...
  <a href="https://matury-online.pl/dashboard" style="display: inline-block; padding: 12px 24px; background: #6366f1; color: white; text-decoration: none; border-radius: 8px; font-weight: 600; margin: 16px 0;">
    Kontynuuj naukę →
  </a>
  <p style="color: #666; font-size: 13px; margin-top: 24px;">Nie chcesz więcej dostawać takich wiadomości? Zmień ustawienia powiadomień w swoim profilu.</p>
      ...
      console.error(`[CRON] Failed to send streak reminder to ${user.email}:`, err);
    }
  }
...
export async function sendWinBackEmails() {
  ...
  for (const user of users) {
    const recentEmail = await prisma.emailLog.findFirst({
      where: { userId: user.id, type: 'win_back', sentAt: { gte: sevenDaysAgo } },
    });
    ...
    try {
      await ses.send(new SendEmailCommand({
        ...
        Destination: { ToAddresses: [user.email] },
        Message: {
          Subject: { Data: `📚 ${user.name || 'Hej'}, Twoja matura się zbliża!` },
          Body: {
            Html: {
              Data: `
<div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
  ...
  <a href="https://matury-online.pl/dashboard" style="display: inline-block; padding: 12px 24px; background: #6366f1; color: white; text-decoration: none; border-radius: 8px; font-weight: 600;">
    Wracam do nauki →
  </a>
</div>`,
            },
...
export async function sendWeeklySummaries() {
  ...
  const activeUsers = await prisma.user.findMany({
    ...
    select: { id: true, email: true, name: true, totalXp: true, currentStreak: true },
    take: 1000,
  });
  ...
  for (const user of activeUsers) {
    ...
    try {
      await ses.send(new SendEmailCommand({
        ...
        Destination: { ToAddresses: [user.email] },
        Message: {
          Subject: { Data: `📊 Twój tygodniowy raport — ${weekAnswers} pytań, ${accuracy}% poprawnie` },
          Body: {
            Html: {
              Data: `
<div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
